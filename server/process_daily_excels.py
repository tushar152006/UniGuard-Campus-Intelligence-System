...
EXCELS_DIR = BASE_DIR.parent / "data" / "daily_excels"
PROCESSED_DIR = EXCELS_DIR / "processed"
...
def append_to_logs(parsed_data_array):
    ...
    print(f"✅ Public Knowledge Graph updated with {len(parsed_data_array)} new records.")

    with open(private_file, "w") as f:
        json.dump(private_logs, f, indent=2)
    print(f"✅ Secure Admin Database updated with {len(parsed_data_array)} new records.")

if __name__ == "__main__":
    print(f"--- Scanning {EXCELS_DIR} for new Excel files ---")
    
    if not EXCELS_DIR.exists():
        print(f"Creating directory: {EXCELS_DIR}")
        os.makedirs(EXCELS_DIR, exist_ok=True)
        
    if not PROCESSED_DIR.exists():
        os.makedirs(PROCESSED_DIR, exist_ok=True)
        
    excel_files = list(EXCELS_DIR.glob("*.xlsx"))
    
    if not excel_files:
        print("No new Excel sheets found.")
        sys.exit(0)
    ...
    for file_path in excel_files:
        ...
        try:
            ...
            print(f"Failed to read Excel file '{file_path.name}': {e}")
            continue
    ...
    print("\n>> Batch Excel Pipeline Complete. UniGuard Chatbot is up-to-date with doctor sheets.")
